...
ui <- dashboardPage(
  ...
  # Application title
  dashboardHeader(title = "Person 2 Vote"),
  dashboardSidebar(
...
server <- function(input, output) {
  ...
  lapply(1:15, function(i){
    ...
    observeEvent(input[[paste0('Comparison',i)]], {
      ...
      rownums <- i +1
      range_write(voting_url, data = as.data.frame(input[[paste0('Comparison', i)]]), range = paste0("B",rownums), col_names = FALSE)
  ...
  lapply(1:10, function(g){
    ...
    observeEvent(input[[paste0('Validation', g)]], {
      rownums1 <- g+17
      range_write(voting_url, data = as.data.frame(input[[paste0('Validation', g)]]), range = paste0("B", rownums1), col_names = FALSE)
  ...
  observeEvent(input$recommendationVote,{
    ...
    range_write(voting_url, data = as.data.frame(input$recommendationVote), range = "B17", col_names = FALSE)
  })
}
