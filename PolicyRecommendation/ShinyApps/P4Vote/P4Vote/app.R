...
ui <- dashboardPage(
  ...
  # Application title
  dashboardHeader(title = "Person 4 Vote"),
  dashboardSidebar(
  ...
  dashboardBody(
    ...
    tabItems(
      tabItem(tabName = "consent",
              ...
              fluidRow(
                box(
                  ...
                  h3("Purpose"), 
                  "The purpose of the research is to understand and improve the negotiation process of a group of experts. 
                  Specifically, the study will evaluate how we can aggregate different expert judgments in the metal additive manufacturing space.",
                  tags$br(),
                  h3("Summary"),
                  "We will ask you to evaluate with other study participants on the suitability of different parts for metal additive manufacturing (MAM).  
                  While in the group, you will be asked to collectively select one part that will best capture the group's preference.",
                  tags$br(),
                  h3("Procedure"),
                  ...
                  In this stage 2 study, you will be in a group with up to 3 other participants. 
                  As a group, you will be asked to determine the most suitable part for MAM. This task will take 30-45 minutes to complete.",
                  tags$br(),
                  tags$br(),
                  ...
                  "Participation in this study is limited to individuals age 18 and older and who are conversational English speakers and who has expertise in either MAM or Army operations.",
                  tags$br(),
                  h3("Risks"),
                  "For this second stage, the risks and discomfort associated with participation in this study are no greater than those ordinarily encountered in daily life or during discussions with colleagues while using a computer. There is a potential risk of breach of confidentiality.",
                  tags$br(),
                  h3("Benefits"),
                  ...
                  tags$br(),
                  tags$br(),
                  "The researchers will take the following steps to protect participants' identities during this study: (1) Each participant will be assigned a number; 
                  (2) The researchers will record any data collected during the study by that number, not by name; (3) Any original data files will be stored in a secured location accessed only by authorized researchers.",
                  tags$br(),
                  h3("Rights"),
      ...
      tabItem(tabName = "voting",
              h2 = "Group Discussion",
              fluidRow(
                box(title = "Policy Characteristic Information",width = 12, collapsible = TRUE, collapsed = TRUE,
                    htmlOutput("inst")  
                )
...
server <- function(input, output) {
  ...
  observeEvent(input$idSubmit,{
    range_write(voting_url, data = as.data.frame(input$idInput), range = "D1", col_names = FALSE)
  ...
  lapply(1:15, function(i){
    ...
    observeEvent(input[[paste0('Comparison',i)]], {
      ...
      rownums <- i +1
      range_write(voting_url, data = as.data.frame(input[[paste0('Comparison', i)]]), range = paste0("D",rownums), col_names = FALSE)
  ...
  lapply(1:10, function(g){
    ...
    observeEvent(input[[paste0('Validation', g)]], {
      rownums1 <- g+17
      range_write(voting_url, data = as.data.frame(input[[paste0('Validation', g)]]), range = paste0("D", rownums1), col_names = FALSE)
  ...
  observeEvent(input$recommendationVote,{
    ...
    range_write(voting_url, data = as.data.frame(input$recommendationVote), range = "D17", col_names = FALSE)
  })
}
