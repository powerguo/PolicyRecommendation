...
server <- function(input, output) {
  ...
  optimal.design <- Dopt.design(13, data = group.utility, 
                                formula = ~p1.utility+p2.utility+p3.utility+p4.utility)
  ...
  sheet_write(final.recommendation, data = optimal.design, "Optimal Design")
  ...
  observeEvent(input$updateAlternatives,{
    ...
    output$RecommendationGroup1 <- renderTable({
      ...
      if(counter_refresh() <=14){
      ...
      }else if(counter_refresh() ==15){
        final.recommendation <- as_sheets_id(final.recommendation)
        optimal.design1 <- sheets_read(final.recommendation, sheet = "Optimal Design", range = "A1:H14")%>%
          left_join(group.utility, by = c("nuc.power1", "nuc.power2", "CO2Price", "ban.fuel.explore1", "ban.fuel.explore2", "RFS1", "RFS2"))%>%
          mutate(id = id.y)%>%
          dplyr::select(!c(id.x, id.y))%>%
          dplyr::select(nuc.power1:RFS2, id, ends_with('utility'), group.score, group.sd)
        
        all.responses <- sheets_read(voting, sheet = 1, range = "A1:D14")
        
        
        
        y.recommendation <- array(dim = c(nrow(all.responses),4))
        
        
        
        for(userResponseCount in 1:nrow(all.responses)){
        ...
        yes <- rowSums(y.recommendation)
        no <- 4-yes
        
        o.choice <- data.frame(yes,no)
        o.choice <- as.matrix(o.choice)
        
        
        
        m1.model <- stan_glm(formula = o.choice ~ 1 + (p1.utility) + (p2.utility) + (p3.utility) + (p4.utility), data = optimal.design1, family = binomial(), 
                             prior = normal())
        
        var.model <- diag(m1.model$covmat)
        var.left <- p1.utility[,2]^2*var.model[2]+p2.utility[,2]^2*var.model[3] + p3.utility[,2]^2 * var.model[4] + p4.utility[,2]^2 * var.model[5]
        
        var.right <- p1.utility[,2]*p2.utility[,2]*m1.model$covmat[2, 3] + p1.utility[,2]*p3.utility[,2]*m1.model$covmat[2, 4] + p1.utility[,2] * p4.utility[,2] * m1.model$covmat[2, 5] +
          p2.utility[,2]*p3.utility[,2]*m1.model$covmat[3, 4] + p2.utility[,2]*p4.utility[,2]*m1.model$covmat[3, 5] + p3.utility[,2]*p4.utility[,2] * m1.model$covmat[4, 5]
        ...
        optimal.design1 <- rbind(optimal.design1, new.alt)
        
        design.left <- optimal.design1%>%
          dplyr::mutate(CO2Price = ifelse(CO2Price == 30, 1, ifelse(CO2Price == 60, 2,ifelse(CO2Price == 90, 3,
                                                                                             ifelse(CO2Price == 120, 4, ifelse(CO2Price == 150, 5, 0))))))%>%
          dplyr::select(1:8)%>%
          dplyr::mutate(nuc.power = ifelse(nuc.power1 == 1, 1, ifelse(nuc.power2 ==1, 2, 0)))%>%
        ...
        sheet_append(data = left.group, ss = left.alternatives, sheet = 1)
        sheet_append(data = right.group, ss = right.alternatives,  sheet = 1)
        sheet_write(ss = final.recommendation, sheet = "Optimal Design", data = optimal.design1)
      ...
      else if(counter_refresh() ==16){
        final.recommendation <- as_sheets_id(final.recommendation)
        
        optimal.design1 <- sheets_read(final.recommendation, sheet = "Optimal Design")
        
        #optimal.design1 <- sheets_read(final.recommendation, sheet = "Optimal Design")
        
        all.responses <- sheets_read(voting, sheet = 1, range = "A1:D15")
        ...
        all.responses <- sheets_read(voting, sheet = 1, range = "A1:D16")
        ...
        sheet_write(ss = predictedprob, data = new.ranking, sheet = 1)
        
        recommendation <- new.alt[1:7]%>%
          dplyr::mutate(nuc.power = ifelse(nuc.power1 == 1, 1, ifelse(nuc.power2 ==1, 2, 0)))%>%
          dplyr::mutate(ban.fuel.explore = ifelse(ban.fuel.explore1 == 1, 1, ifelse(ban.fuel.explore2 ==1, 2, 0)))%>%
          dplyr::mutate(RFS = ifelse(RFS1 == 1, 1, ifelse(RFS2 == 1, 2, 0)))%>%
          dplyr::select(nuc.power, CO2Price, ban.fuel.explore, RFS)%>%
        ...
        sheet_write(ss = final.recommendation, sheet =1, data = recommendation)
    ...
    output$validationQuestion <- renderTable({
      if(counter_refresh() <= 17){
        ...
        sheet_write(final.recommendation, sheet = "Random Questions", data = random.questions.reformed)
        sheet_write(final.recommendation, sheet = "Raw Random", data = random.questions)
        ...
        validation.final <- sheets_read(final.recommendation, sheet = 1)
